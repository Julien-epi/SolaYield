...
class SolaYieldDeployedInteract {
  program: Program<any>;
  ...
  // 5. Obtenir les infos d'une stratégie
  ...
  // 7. Obtenir le nombre de stratégies
...
// CLI Usage
async function main() {
  const cli = new SolaYieldDeployedInteract();
  const args = process.argv.slice(2);
  
  if (args.length === 0) {
    console.log(`
🌾 SolaYield CLI - Deployed Contract Interface
📋 Contract: BCz6K4XSaycH954PhZPPmwuistSyJP5p5Biya7frA2Az (Devnet)

Usage: ts-node scripts/deployed-interact.ts <command> [args]
...
Examples:
  ts-node scripts/deployed-interact.ts init
  ts-node scripts/deployed-interact.ts create-strategy "SOL Staking" 1000 0
  ts-node scripts/deployed-interact.ts deposit 0 1.5
  ts-node scripts/deployed-interact.ts claim 0
  ts-node scripts/deployed-interact.ts strategy-info 0
    `);
    return;
  }
...
export { SolaYieldDeployedInteract }; 