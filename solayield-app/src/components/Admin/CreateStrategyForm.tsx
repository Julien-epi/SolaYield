...
const CreateStrategyForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // APY en basis points comme BN (maintenant u64 dans l'IDL !)
      const apyBasisPointsBN = new anchor.BN(parseInt(apy));
      ...
      if (err.logs) {
        ...
        try {
          const anchorLogs = await err.getLogs();
          logs = "\nLogs:\n" + anchorLogs.join("\n");
        } catch {}
      }
      alert("Erreur lors de la création : " + (err.message || err) + logs);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form
      ...
      className="bg-[#181c25] p-8 rounded-xl shadow-lg flex flex-col gap-6 w-full border border-[#23283a]"
    >
      <label className="text-sm font-medium text-[#b0b8d1]">
        Nom de la stratégie
        <input
          type="text"
          value={name}
      ...
      <button
        type="submit"
        className="w-full bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-3 px-4 rounded-lg transition text-lg mt-2 shadow focus:outline-none focus:ring-2 focus:ring-indigo-400"
        disabled={loading}
      >
